...
<head>
    ...
    <title>ประวัติการให้การสงเคราะห์ค่าคลอดบุตร </title>
    <!-- Favicon icon -->
                ...
                <div class="row">
                    <div class="col-xl-12">
                        <div class="page-title-content">
                            <p>ประวัติการให้การสงเคราะห์ค่าคลอดบุตร
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-md-12">
                        <?php
                        $sql = "
                        SELECT * FROM req_maternity as rmat
                        INNER JOIN tbl_member as m ON m.m_id = rmat.m_id 
                        INNER JOIN tbl_status as st ON st.s_id = rmat.s_id 
                        WHERE rmat.m_id = " . intval($_SESSION["m_id"]) . " AND m.m_alive <> 0
                        ORDER BY rmat.m_id DESC";
                    ...
                    <th width='8%'>ดูรายละเอียด</th>
                        ...
                        while ($row = $db->getData()) {
                            echo "<tr>";
                            echo "<td>" . $row["REQ_MAT_ID"] .  "</td> ";

                            echo "<td>" . $row["REQ_MAT_DATE"] .  "</td> ";
                            echo "<td>" . $row["m_fname"] . $row["m_name"] . ' ' . $row["m_lname"] . "</td> ";
                            echo "<td>" . $row["REQ_MAT_VALUE"] .  "</td> ";
                            echo "<td>" . $row["REQ_MAT_VALUE_APPROVE"] .  "</td> ";
                            echo "<td>" . $row["s_name"] .  "</td> ";
                            echo "<td><a href='mat_view.php?&REQ_MAT_ID=".$row['REQ_MAT_ID']."' class='btn btn-primary mt-3 waves-effect'>ดูรายการ</a> 
    <br></br>
