...
if ($_POST['type_finan']) {
    $sql = "UPDATE req_death SET s_id=$s_id WHERE m_id= '$m_id'";
    $db->Execute($sql);
    ...
    $REQ_DEATH_AGE = $_POST['REQ_DEATH_AGE'];
    $REQ_D_DATE = date('d/m/Y h:i:s', strtotime("+543 years"));
    ...
    $now = date('d-m-Y', strtotime('+543 years'));
    $sql = "UPDATE veteran  SET VT_ALIVE='0' , VT_DEAD_DATE='$REQ_DEATH_DATE' , VT_DEAD_REASON='$REQ_DEATH_CAUSE' WHERE m_id = '$m_id' ";
    $db->Execute($sql);
...
if ($_SESSION['m_level'] != "finoffice") {
    ...
    $(document).ready(function(){
        ...
        setTimeout(function(){window.location="death_list.php?type=9"}, 2000);
    });
    </script>
    ';
}
